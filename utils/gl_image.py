...
def GetFirmwareInfo(f, config):
    ...
    print("%14s  %-40s %-34s %-10s %-30s" % (
    str_model, str_firmware_name, str_firmware_md5sum, str_firmware_size, str_ib))
    for file in files:
        ...
        print("%14s  %-40s %-34s %-10s %-30s" % (
        file, subfiles[0], md5sum, str(os.path.getsize(f + "/" + file + "/" + subfiles[0])), ib))
...
class Config:
    ...
    def parse(self):
        ...
        try:
            ...
            print
            "Oops! Failed to parse %s" % self.filename
            sys.exit(1)
...
def main(argv):
    ...
    try:
        (opts, args) = getopt.getopt(argv[1:],
                                     "ap:e:b:f:c:lniho:j:",
                                     ["all", "profile=", "extra=", "branch=", "files=", "config=", "list", "clean",
                                      "ignore", "help", "output", "offline", "json="])
    ...
    if not os.path.isdir(dir_name):
        ...
        check_call("git clone %s %s" % (gl_inet_ipks_url, dir_name), shell=True)
    ...
    for image in images:
        if c.getDisabled(image):
            ...
            continue
        if c.getImagebuilderVersion(image) != c.getImagebuilderName(image).split('_')[
            len(c.getImagebuilderName(image).split('_')) - 1]:
            print(
        ...
        try:
            print('===============********************++++++++++++++++=====================')
            print(im_path)
            print("make -f rules.mk val.BOARD V=s 2>/dev/null")
        ...
        try:
            ...
            if c.getState(image) == "develop":
                os.system(
                    "echo 'Pls do not use the firmwares in this folder until this file is removed.' >  " + bin_dir + "/Do\ not\ use\ until\ this\ file\ is\ removed.txt")
            else:
            ...
            if c.getProfile(image) == "QSDK_Premium":
                if image == "b1300" or image == "b1300_2b":
                    ...
                    if output != "":
                        fw_name = bin_dir + "/" + "qsdk-" + image + "-" + c.getVersion(image) + "-" + time.strftime(
                            "%m%d", time.localtime()) + os.path.splitext(output)[-1]
                ...
                if filename != "glinet/images.json":
                    fw_name = bin_dir + "/" + "openwrt-" + image + "-" + c.getVersion(image) + "-" + time.strftime(
                        "%m%d", time.localtime()) + os.path.splitext(output)[-1]
                else:
                    fw_name = bin_dir + "/" + "openwrt-" + image + "-" + c.getVersion(image) + "-" + time.strftime(
                        "%m%d", time.localtime()) + os.path.splitext(output)[-1]
                ...
                if image == "sft1200":
                    check_call("find bin/ -name \"*%s-squashfs-sysupgrade*\" | xargs cp -t %s/" % (image, bin_dir),
                               shell=True, cwd=im_path)
                    return_code, output = commands.getstatusoutput(
                    ...
                    check_call("find bin/ -name \"*%s*squashfs-factory*\" | xargs cp -t %s/" % (image, bin_dir),
                               shell=True, cwd=im_path)
                    return_code, output = commands.getstatusoutput(
                    ...
                    check_call(
                        "find bin/ -name \"*%s-siflower-sf19a28-fullmask-squashfs-sysupgrade*\" | xargs cp -t %s/" \
                ...
                if c.getProfile(image).find('nand') != -1 or c.getProfile(image).find(
                        'xe300-iot') != -1 or c.getProfile(image).find('axt1800') != -1 or c.getProfile(image).find(
                        'ax1800') != -1:
                    if image == "e750-1907":
                        check_call("find bin/ -name \"*%s*squashfs-factory*\" | xargs cp -t %s/" % (image, bin_dir),
                                   shell=True, cwd=im_path)
                    ...
                    # else image == "xe300":
                        ...
                        check_call("find bin/ -name \"*%s*-factory.img\" | xargs cp -t %s/" % (image, bin_dir),
                                   shell=True, cwd=im_path)
                        return_code, output = commands.getstatusoutput(
            ...
            print
            "Failed to build %s..." % c.getProfile(image)
            continue
